...
type result =
  ...
  | Rrel of string * int64 (* relative label + offset *)
...
let eval_const b current_pos cst =
  ...
  let rec eval = function
    ...
    | ConstAdd (c1, c2) ->
        ...
        | Rabs (s, n1), Rint n2
        | Rint n2, Rabs (s, n1) -> Rabs (s, Int64.add n1 n2)
        | Rrel (s, n1), Rint n2
        | Rint n2, Rrel (s, n1) -> Rrel (s, Int64.add n1 n2)
...
let rex_of_reg8 = function
  | Reg8L (RSP | RBP | RSI | RDI) -> rex
  | _ -> 0
...
let buf_sym b sym offset =
  ...
  | Some lbl -> (* TODO: assert we are in 32 bits ? *)
...
let emit_mod_rm_reg b rex opcodes rm reg =
  ...
  | Mem64_RIP (_, symbol, offset) ->
      ...
      record_reloc b (Buffer.length b.buf) (RELOC_REL32 (symbol, Int64.of_int offset));
      buf_int32L b 0L
  ...
  | Mem {arch; typ = _; idx; scale; base; sym; displ} ->
      let offset =
        ...
        match sym with
        | None ->
            if is_imm8L displ then OImm8 displ else
            if is_imm32L displ then OImm32 (None, displ)
            else assert false
      ...
      end else begin match base with
      | None ->
          begin match idx_reg, scale, offset with
          | (RSP|R12), 1, OImm8 0L ->
      ...
      | Some base_reg ->
          ...
          emit_rex b (rex lor (rexr_reg reg) lor (rexx_index idx) lor (rexb_base base));
          buf_opcodes b opcodes;
          begin match base_reg, offset with
          | (RBP|R13), OImm8 0L -> (* to 0x00(%rbp+reg) *)
              buf_int8 b (mod_rm_reg 0b01 0b100 reg);
...
let emit_MOV b dst src =
  ...
  | (Mem _ | Mem64_RIP _  as rm) , (Reg8L _ | Reg8H _ as reg) ->
      emit_mod_rm_reg b (rex_of_reg8 reg)
  ...
  | Reg64 r64, Imm n when not (is_imm32L n) -> (* MOVNoneQ *)
...
let emit_simple_encoding enc b dst src =
  ...
  | { rm64_imm8 = opcodes; reg },
    (Reg64 _ | Mem {typ=(NONE|QWORD|REAL8); arch=X64}  as rm), Imm n when is_imm8L n ->
      emit_mod_rm_reg b rexw opcodes rm reg;
      buf_int8L b n

  | { rm8_imm8 = opcodes; reg },
    (Reg8L _ | Reg8H _ | Mem {typ=BYTE; arch=X64}  as rm), Imm n ->
      assert(is_imm8L n);
      emit_mod_rm_reg b rexw opcodes rm reg;
      buf_int8L b n
  ...
  | { rm64_imm32 = opcodes; reg },
    (Reg32 _
    | Mem {typ=NONE; arch=X86}
    | Mem {typ=(DWORD|REAL4)} as rm),  (Imm _ | Sym _ as n) ->
      emit_mod_rm_reg b 0 opcodes rm reg;
      buf_int32_imm b n
...
let emit_shift reg b dst src =
  ...
  | _ ->
      Format.eprintf "emit_shift: src=%a dst=%a@." print_old_arg src print_old_arg dst;
      assert false
...
let emit_jmp b loc dst =
  match dst with
  | Sym symbol ->
      emit_reloc_jump [0xEB] [0xE9] b loc symbol
...
let emit_inc b = function
  | Reg64 _ | Reg32 _ | Mem _ | Mem64_RIP _  as rm -> emit_mod_rm_reg b rexw [0xFF] rm 0
  | _ -> assert false


let emit_DEC b = function
  (* FE /1 DEC r/m8 M Valid Valid *)
  | [ Reg8L _ | Reg8H _ | Mem {typ=BYTE} as rm ] -> emit_mod_rm_reg b no_rex [0xFE] rm 1
  (* FF /1 DEC r/m16 M Valid Valid *)
  | [ Reg16 _ | Mem {typ=WORD} as rm ] -> emit_mod_rm_reg b no_rex [0x66; 0xFF] rm 1
  (* FF /1 DEC r/m32 M Valid Valid *)
  | [Reg32 _ | Mem {typ=DWORD}  as rm] -> emit_mod_rm_reg b no_rex [0xFF] rm 1
  (* REX.W + FF /1 DEC r/m64 M Valid N.E. *)
  | [Reg64 _ | Mem {typ=QWORD}  as rm] -> emit_mod_rm_reg b rexw [0xFF] rm 1
  | _ -> assert false
...
let emit_FNSTCW b = function
  | Mem {typ=(NONE|WORD)} as rm -> emit_mod_rm_reg b no_rex [0x9B;0xD9] rm 7
  | _ -> assert false
...
let emit_XCHG b src dst = (* TODO: test ! *)
  match dst, src with
  | ((Reg64 _ | Mem _ | Mem64_RIP _) as rm), Reg64 reg
  | Reg64 reg, (Mem _ | Mem64_RIP _ as rm) -> (* r64, r/m64 *)
      emit_mod_rm_reg b rexw [ 0x87 ] rm (rd_of_reg64 reg)
  | ((Reg32 _ | Mem _ | Mem64_RIP _) as rm), Reg32 reg
  | Reg32 reg, (Mem _ | Mem64_RIP _ as rm)  -> (* r32, r/m32 *)
      emit_mod_rm_reg b no_rex [ 0x87 ] rm (rd_of_reg64 reg)
  | ((Reg16 _ | Mem _ | Mem64_RIP _) as rm), Reg16 reg
  | Reg16 reg, (Mem _ | Mem64_RIP _ as rm)  -> (* r16, r/m16 *)
      emit_mod_rm_reg b rex [ 0x66; 0x87 ] rm (rd_of_reg64 reg)
  | ((Reg8L _ | Reg8H _ | Mem _ | Mem64_RIP _) as rm), (Reg8L _ | Reg8H _ as reg)
  | (Reg8L _ | Reg8H _ as reg), (Mem _ | Mem64_RIP _ as rm)  -> (* r8, r/m8 *)
...
let assemble_line b loc ins =
  try
    match ins with
    | Ins instr -> assemble_instr b loc instr; incr loc
    | Comment _ -> ()
    | Global s -> (get_symbol b s).sy_global <- true

    | Quad (Const n) -> buf_int64L b n
    | Quad cst -> record_local_reloc b (RelocConstant (cst, B64)); buf_int64L b 0L

    | Long (Const n) -> buf_int32L b n
    | Long cst -> record_local_reloc b (RelocConstant (cst, B32)); buf_int32L b 0L

    | Word (Const n) -> buf_int16L b n
    | Word cst -> record_local_reloc b (RelocConstant (cst, B16)); buf_int16L b 0L

    | Byte (Const n) -> buf_int8L b n
    | Byte cst -> record_local_reloc b (RelocConstant (cst, B8)); buf_int8L b 0L
...
let assemble_section arch section =
  ...
  Array.iter
    (function
       | NewLabel (lbl, _) -> local_labels := String.Map.add lbl !icount !local_labels
       | Ins _ -> incr icount
       | _ -> ()
    )
    section.sec_instrs;
